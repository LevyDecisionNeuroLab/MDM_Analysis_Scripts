...
fitparwave = 'Behavior data fitpar_04272017\';
prtwave = 'Prt_Model_disp_resp_05052017\';
...
ParametricModType = {'RewardValue','SV','CV','CR'}; % CV means Chosen Subjective value, CR means Chosen Reward magnitude
...
for i = 1:length(SubjectNums)
    for j = 1:length(domain)
        for k = 1:length(ParametricModType)
            ...
            if strcmp(domain{j},'mon') % skip medical blocks, these parameters are meanlingless
                PTB_MDM_Protocol_Gen_ver8_1(SubjectNums(i), domain{j}, ...
