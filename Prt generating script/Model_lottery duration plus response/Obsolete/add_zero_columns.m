...
sdmwave = 'SDM_Model_disp_resp_04182017';
...
for i = 1:length(ParametricModType)
    ...
    %         sdm_name = ['Prt files' num2str(subject) '_S' num2str(session) '_block' num2str(run) '_' gainloss num2str(block) '_type_AmbiguityLevel.sdm']; % change for RiskLevel
            ...
            PredictorNames15 = sdm.PredictorNames;
            PredictorNames17 = {'Amb_mon_Display', 'Amb_mon_Display x p1', 'Amb_mon_Resp', 'Amb_mon_Resp x p1',...
                                'Risk_mon_Display', 'Risk_mon_Display x p1', 'Risk_mon_Resp', 'Risk_mon_Resp x p1', ...
                                'Amb_med_Display', 'Amb_med_Display x p1', 'Amb_med_Resp', 'Amb_med_Resp x p1', ...
                                'Risk_med_Display', 'Risk_med_Display x p1', 'Risk_med_Resp', 'Risk_med_Resp x p1', ...
                                'Constant'};
            sdm.PredictorNames = PredictorNames17;

            % find the missing column
            miss = 0; %How many columns are missing
            for m = 1:length(PredictorNames17)
                exist = 0;
                for n = 1:length(PredictorNames15)
                     if strcmp(PredictorNames15(n), PredictorNames17(m))
            ...
            a = zeros(300,17); % volume number x condition numbers
            i=1; %column number for new SDM (length = 17)
            j=1; %column number for old SDM (length = 15)
            while i<18
