...
% seperate for risk and ambig
% display + resposne
% so it should be 2x4x2=16 predicotrs for each domain,
...
PRT.NrOfConditions =      '32'; % NOTE: this is re-written for each ParametircModType in the loop below. 
...
for i = 1:length(SubjectNums)
    for j = 1:length(domain)
        for k = 1:length(ParametricModType)
            % for non parametric design matrics(4magnitude*2risk/ambig*2domains=16 lottery duration + 16 responses)
            if strcmp(ParametricModType{k}, '')
                PRT.NrOfConditions =      '32';
%             else
...
%                 PRT.NrOfConditions =      '12'; % not sure if this situation need parametirc modulators
            ...
            if (strcmp(ParametricModType{k},'SV')|strcmp(ParametricModType{k},'RewardValue')) & strcmp(domain{j},'med') % for medical blocks, these two parameters are meanlingless
                ...
                PTB_MDM_Protocol_Gen_ver4(SubjectNums(i), domain{j}, ...
