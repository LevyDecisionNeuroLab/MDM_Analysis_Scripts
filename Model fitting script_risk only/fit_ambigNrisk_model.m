...
function [info,p] = fit_ambigNrisk_model(choice,vF,vA,pF,pA,model,b0,base)

% If multiple model fits requested, loop and pack 
if iscell(model)
   for i = 1:length(model)
      [info(i),p{i}] = fit_ambigNrisk_model(choice,vF,vA,pF,pA,model{i},b0,base);
   end
   return;
end
...
for i = 1 : size(b0,1)
    ...
    if exist('fminunc','file')
       try
          ...
          OPTIONS = optimset('Display','off','LargeScale','off');
              [b,negLL,exitflag,convg,g,H] = fminunc(@local_negLL,b00,OPTIONS,choice,vF,vA,pF,pA,model,base);
    ...
    if strcmp(optimizer,'fminsearch')
       ...
       [b,negLL,exitflag,convg] = fminsearch(@local_negLL,b00,OPTIONS,choice,vF,vA,pF,pA,model,base);
    end
    ...
    if i == 1 || (i ~=1 && LL > info.LL)% first iteration; and if a later iteration renders larger likelihood, replace info
        ...
        p = choice_prob_ambigNrisk(base,vF,vA,pF,pA,b,model);
...
function sumerr = local_negLL(beta,choice,vF,vA,pF,pA,model,base)
% estimated likelihood of chooseing the lottery
p = choice_prob_ambigNrisk(base,vF,vA,pF,pA,beta,model); 
